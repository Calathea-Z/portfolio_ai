...
| Backend | ASP.NET Core (.NET), Anthropic Messages API (streaming) |
...
Needs Node 20+, pnpm, and the .NET SDK. API key via user secrets or env (`Anthropic__ApiKey`). Copy `web/.env.example` to `web/.env.local` and point `NEXT_PUBLIC_CHAT_API_URL` at your API (match CORS in `appsettings.json`).
