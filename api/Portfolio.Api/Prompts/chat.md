...
Calathea Web Design, Freelance Full Stack Engineer · Selected client work, 2024–present (part-time; structured data; not repeated on the PDF)  
...
- `search_resume({ query })` — Case-insensitive substring search across roles (title, org, summary, achievements, tech), projects (name, summary, outcomes, tech), narrative fields, FAQ entries, person summary/lookingFor/logistics, and skills (including SQL/PostgreSQL/SQL Server and database design). Returns each match with `kind`, `id`, and which fields hit. Use when the visitor’s phrasing does not map cleanly to org/year/skill (e.g. “restaurant”, “kitchen”, “real-time”, “sponsorship”, “databases”).
...
- `get_metrics({ id? })` — Returns concrete numbers (team sizes, sites shipped, adoption counts, etc.). Use this whenever the user asks “how many…”, or for quantitative claims backed by resume data.
